...
static u32 __iomem *virt_io_base;
...
static int dummy_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_RESUME:
        	iowrite32be((u32)virt_to_phys(runtime->dma_area), virt_io_base + 0x1);
        	//iowrite32be((u32)virt_to_phys(runtime->dma_addr), virt_io_base + 0x1);
        	//iowrite32be((u32)virt_to_phys(runtime->dma_buffer_p), virt_io_base + 0x1);
        	//iowrite32be((u32)(runtime->dma_buffer_p), virt_io_base + 0x1);
        	//iowrite32be((u32)(runtime->dma_bytes),            virt_io_base + 0x1);
		//iowrite32be((u32)virt_to_page(dummy_page[substream->stream]), virt_io_base + 0x1);
                iowrite32be(runtime->periods,                     virt_io_base + 0x2);
                iowrite32be(runtime->period_size,                 virt_io_base + 0x3);
                iowrite32be(runtime->rate,                        virt_io_base + 0x5);
                iowrite32be(runtime->channels,                    virt_io_base + 0x6);
                iowrite32be(runtime->format,                      virt_io_base + 0x7);
                iowrite32be(0x1,                                  virt_io_base + 0x0);
		return dummy->timer_ops->start(substream);
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
                iowrite32be(0x0, virt_io_base + 0x0);
...
static int snd_dummy_probe(struct platform_device *devptr)
{
	...
	virt_io_base = ioremap_nocache(0x98000000, 0x400);
	//u32 c = ioread32be(tc_base + 0x0);
